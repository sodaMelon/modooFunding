...
<html>
<head>
<script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-5c511de577f15824"></script> 
<meta charset="UTF-8">
...
<body>
   <div id="con">
   <form name=form2 action="projectlist" method=post><input type="hidden" name="hiddenid" id="hiddenid" value="${project.pj_id}"> </form>
      <form name="form" id="detail" method="post">
         ...
         <c:if test="${userId eq project.pj_id}">
         	<a id=fb1>관리자로부터 받은 검토 피드백</a><br>
         		<div class="feedback">
         			<a id=fb2>${project.feedback}</a><br>
         		</div>
         <br>
         </c:if>
        ...
        <c:choose>
			<c:when test="${project.c_status == 1}">
				<c:choose>
					<c:when test="${funder.payment == 1}">
						<input type="button" class="funding1" value="이미 펀딩하셨습니다." onclick="go_mine()">
         ...
         <c:choose>
            <c:when test= "${project.c_status == 1 && user == 0}"> 
            </c:when>
            <c:when test="${project.c_status == 1 && user != 0 && like.like_yn == 0}"> 
               <img src="images/dislike.png" id="heart" width="40px" onclick="like()"><b id=likecount>${likecount}</b>
             </c:when>
            <c:when test="${project.c_status == 1 && user != 0  && like.like_yn == 1}">
               <img src="images/like.png" id="heart"  width="40px" onclick="like()"><b id=likecount>${likecount}</b>
            </c:when>
            <c:when test="${project.c_status == 1 && user != 0  && like.like_yn == null}">
               <img src="images/dislike.png" id="heart"   width="40px" onclick="like()"><b id=likecount>${likecount}</b>
            </c:when>
            <c:when test="${project.c_status != 1}">
            </c:when>
         ...
         <div id="planner">
            ...
            &nbsp;<a style="color:black; vertical-align:4.5px;" id="golist" onclick="goPjList()"><%-- href="<c:url value="projectlist${project.pj_id}"/>"> --%>${project.pl_name}</a>
                </div>
